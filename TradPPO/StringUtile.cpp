...
#include <algorithm>
  ...
  void StringUtile::Replace(std::string& _str, const char& _old, const char& _newValue)

{
	 std::ranges::replace(_str, _old,_newValue);
	
}
  void StringUtile::Replace(std::string& _str, const std::string& _old, const std::string& _newValue)
  {
	  size_t _startPos = 0;
