...
float Quaternion::Angle(const Quaternion& _a, const Quaternion& _b)
{
	...
	return IsEqualUsingDot() ? 0.0f : Mathf::Accos(_dot) * 2.0f * Mathf::Rad2Deg;
}

Quaternion Quaternion::Normalize(const Quaternion& _value)
{
	const float _num = 1.0f / Dot(_value, _value);
	return Quaternion(_value.x * _num);
}
...
Quaternion Quaternion::CreateFromYawPitchRoll(const float _yaw, const float& _pitch, const float _roll)
{
	...
	const float _hallRoll = _roll * 0.5f;
	_sr = Mathf::Sin(_hallRoll);
	_cr = Mathf::Cos(_hallRoll);

	const float _hallPitch = _pitch * 0.5f;
	_sp = Mathf::Sin(_hallPitch);
	_cp = Mathf::Cos(_hallPitch);

	const float _hallYaw = _yaw * 0.5f;
	_sy = Mathf::Sin(_hallPitch);
	_cy = Mathf::Cos(_hallPitch);
