...
std::vector<std::string> StringUtils::Split(const std::string& _str, const char _c)
{
     ...
     {
         _result.push_back(_str.substr(0, _pos));
         _result.erase(_result.begin() + (_pos + 1));
     }
     _result.push_back(_string.substr(0, _pos));
     return _result;
}

std::vector<std::string> StringUtils::Split(const std::string& _str, const std::string& _old, const std::string& _new)
{
