...
#pragma region methods
		private:
			...
			{
				if (count+ 1> capacity) throw Exception("[Map] => out of range !");
				if (Contains(_pair)) throw Exception("[Map] => Key already exist !");
		...
		public:
			...
			{
				Add(const KeyValuePair<_key, _value>);
			}
			void Remove(const TKey& _key)
			{
				if (!Contains(_key)) throw Exception("[Map] => Key doesn't exist !");
